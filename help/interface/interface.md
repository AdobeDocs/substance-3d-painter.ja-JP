---
helpx_url: "https://helpx.adobe.com/substance-3d-painter/interface.html"
breadcrumb-title: ''
description: Substance 3D Painterのインターフェイスについて説明し、効率的なテクスチャペインティングのワークフローを実現するためのパネル、メニュー、ツールについて説明します。
helpx_creative_field: ""
helpx_description: Painter > Interface
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: インターフェイス
user-guide-description: ''
user-guide-title: ''
source-git-commit: 9f20406f682e0e6a2e9a423e81c5ecfc7430ecfd
workflow-type: tm+mt
source-wordcount: '70'
ht-degree: 27%

---


# インターフェイス

インターフェイスはいくつかのパネルに分割されており、並べ替えたり、サイズを変更したり、非表示にしたりできます。

アプリケーションのメインウィンドウのリストを次に示します。

* [アセット](../interface/assets/assets.md)
* [カラーピッカー](color-picker.md)
* [表示設定](../interface/display-settings/display-settings.md)
* [ヒストリー](history.md)
* [レイヤースタック](../interface/layer-stack/layer-stack.md)
* [メインメニュー](../interface/main-menu/main-menu.md)
* [プロジェクト設定](project-configuration.md)
* [プロパティ](properties.md)
* [設定](../interface/settings/settings.md)
* [シェーダー設定](../interface/shader-settings/shader-settings.md)
* [テクスチャセット](../interface/texture-set/texture-set.md)
* [ツールバー](toolbars.md)
* [ビューポート](../interface/viewport/viewport.md)

追加のページ：

* [ショートカット](settings/shortcuts.md)
* [スライダー](miscellaneous/sliders.md)
