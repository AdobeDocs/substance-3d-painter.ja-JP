---
helpx_url: "https://helpx.adobe.com/substance-3d-painter/release-notes.html"
breadcrumb-title: ''
description: Substance 3D Painterのリリースノートにアクセスすると、新機能、機能向上、各種バージョンのバグ修正に関する最新情報を参照できます。
helpx_creative_field: ""
helpx_description: Painter > Release notes
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: リリースノート
user-guide-description: ''
user-guide-title: ''
source-git-commit: 19578e843e2843a708e479ec81d94fd8c48c3648
workflow-type: tm+mt
source-wordcount: '70'
ht-degree: 8%

---


# リリースノート

各バージョンについて追加、変更、修正されたすべての項目を検索します。

* [すべての変更](all-changes.md)
* [バージョン 12.1](version-12-1.md)
* [バージョン12.0](version-12-0.md)
* [バージョン11.1](version-11-1.md)
* [最新ベータ版](beta.md)
* [既知の問題](known-issues.md)

最初のバージョン以降にソフトウェアで発生したすべての変更の一覧については、[すべての変更](all-changes.md)ページを参照してください。
