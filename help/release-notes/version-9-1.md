...
<b>Substance 3D Painter 9.1</b>では、SVGツールの正接コントロール、パスファイルフォーマットのサポート、ドラッグ&amp;ドロップによるリソースの読み込みと適用の機能、およびビューポートのtranslucencyのサポートが追加されています。

リリース日： *2023年11月7日*

## 主な機能

### パスツールの新しい正接コントロールと機能強化

![バナーパスツール](../assets/v91_banner_tangents.jpg)

この新しいバージョンでは、コミュニティから要求された欠落ビットや機能を追加するためのPathツール（バージョン9.0で導入）の開発を継続しています。

* <b>コントロールパスが正接を手動でポイントしています</b>

  パス上の特定のポイントの正接を手動で設定できるようになりました。 これにより、自動動作を無効にして新しいシェイプを作成できます。

  ![カスタム正接](../assets/path_control_tangents.gif)
* <b>マニピュレーターを使用してパスポイントを編集する</b>

  オブジェクトの表面のスライディングポイントだけでは不十分な場合があります。 マニピュレーターを使用すると、サーフェスを超えてポイントを動かすことができます。 これは、メッシュの再読み込み後にポイントがサーフェスから離れすぎた場合などに、一度に複数のポイントを動かすのに非常に役立ちます。

  ![パスの正接はマニピュレーターを移動します](../assets/path_tangents_translate.gif)
* <b>パスの表示を個別に切り替える</b>

  専用のビューポートパネルを使用して、パスごとにパスの表示/非表示を変更できるようになりました。 パスを無効にすると、最終的なパスを削除しなくても、そのテクスチャへの影響が取り除かれます。
...
新しいプロジェクトを作成するときに既定で使用される<b>Adobe Standard Material</b> (ASM) シェーダーは、<b>Translucency</b>、<b>透明度</b>および<b>吸収</b>のプロパティをサポートするように更新されました。 つまり、これらのレンダリングビヘイビアーの結果を、リアルタイムビューポート（およびIrayレンダラー内）で表示できるようになりました。
...
* <b>新しいASM シェーダー設定</b>
  ...
  * <b>透明度</b> （不透明度）：葉などの透明なサーフェスを取得するために、別のシェーダーに切り替える必要がなくなりました。 代わりに、<b>ジオメトリ/不透明度</b>グループで、<b>アルファテスト</b>または<b>アルファブレンド</b>のいずれかのパラメーターを有効にします。 ディザリングなどの通常の設定も使用できます。
  * <b>透光性</b> ：この新しいプロパティを使用すると、ガラスのようなサーフェスを作成して、Specularの反射を維持しながらシェイプを透明にすることができます。 プロジェクトにTranslucencyチャンネルを追加し、<b>Interior</b>グループの<b>Translucency</b>パラメーターを有効にするには、このパラメーターを使用します。
...
* <b>Adobe Standard Materialのデモを行う新しいサンプルプロジェクト</b>

  新しいASMプロパティの操作は、最初は難しい場合があるため、シェーダーのいくつかの機能を示す新しいサンプルプロジェクトが追加され、学習が容易になりました。
...
* <b>Translucencyチャンネルの既定値が黒に設定されました</b>

  新しいシェーダープロパティを使いやすくし、ビューポートで予期しない結果が生じないようにするために、チャンネルTranslucencyの既定値の色が（白ではなく）黒に変更されました。

  このチャンネルがプロジェクトで既に使用されている場合は、レイヤースタックの下部に塗りつぶしレイヤーを追加し、チャンネル値を白に設定するだけで、以前の動作を取得できます。 シェーダーパラメーターで<b>[translucencyを散布マスクとして使用]</b>の設定を有効にして、チャンネルの効果をサブサーフェスの散布の結果に再適用することもできます。

### ベクターグラフィックファイル(SVG)の新しいサポート
...
* <b>SVGファイルのインポート</b>\
  ...
  このバージョンでも読み込みが簡単になったため（下記参照）、Painterの外部からメッシューやレイヤースタックにリソースを直接ドラッグ&amp;ドロップするだけで、SVGファイルを使用できます。
* <b>専用SVGの設定</b>\
...
* <b>新しいSVGのカスタマイズされたマテリアル</b>

  テクスチャリング時のSVGファイルの使用に役立つ3つの新しいリソースが追加されました。

  * <b>カスタムスプレーペイント</b>: 1つの入力画像で壁に塗られたデカールをシミュレートします。
  * <b>カスタムステッカー</b>：表面にプラスチックのステッカーを作成します。 いくつかの設定を使用して、ダメージや折り畳みをシミュレートします。
  * <b>グラフィックからマテリアルへ</b>：単一の画像入力から複数のマテリアルプロパティを作成できます。 SVGファイルをビューポートにドラッグ&amp;ドロップすると、このリソースが自動的に挿入されます。 このリソースを使用すると、複数のチャンネルで入力の透明度を簡単に共有できるため、シンプルなデカールに最適です。
...
* <b>ドラッグ&amp;ドロップでレイヤースタックに読み込む</b>\
  外部リソースファイルをリソースにドロップするのと同じように、ファイルをビューポートにドロップすると、レイヤースタック内のリソースを使用してレイヤーやエフェクトを直接作成できます。
* <b>ドラッグアンドドロップでリソーススロットにインポートする</b>

  レイヤーまたはツールにリソースを直接読み込むこともできます。 適切な設定の塗りつぶしレイヤーまたはエフェクトが既に存在する場合は、外部ファイルをプロパティウィンドウのいずれかのチャンネルスロットにドロップして、読み込んで適用します。
...
* <b>多くの種類のリソースをドラッグアンドドロップします</b>
  ...
  リソースの種類をビューポートまたはレイヤースタックに直接ドラッグ&amp;ドロップできるようになりました。 次のタイプのリソースは、どこにでも（ほぼ）ドラッグ&amp;ドロップできます。
...
* <b>ドラッグ中にコンテンツまたはマスク効果スタックを選択\
  ...
  サムネール上にリソースをドラッグすると、Painterは関連するエフェクトスタックに自動的に切り替わります。 その後、そのスタック内の正確な場所にリソースをドロップすることは非常に簡単になります。 これにより、事前に適切なスタックに切り替える必要がなくなります。

  ![エフェクトスタックの切り替え](../assets/v91_drop_stack_switch.gif)
* <b>その場で新しい黒のマスクを作成する</b>

  リソースをドラッグすると、マスクのないレイヤーに新しいアイコンが表示されます。 リソースがこのゴーストマスクにドロップされると、新しいマスクが自動的に作成され、新しいリソースが追加されます。 これは、新しいマスクを簡単に設定し、ドラッグ&amp;ドロップをキャンセルして手動で追加しないようにする方法です。

  ![ドラッグ&amp;ドロップして新しいマスクを作成します](../assets/v91_drop_new_mask.gif)
* <b>ビューポートにドロップして新しいレイヤーを作成</b>

  ビューポートでリソースのドラッグ&amp;ドロップを行って、新しいレイヤーを作成することもできます。 リソースの種類に応じて、結果が変わる場合があります。 フィルターはパススルーモードでペイントレイヤーを作成し、スマートマスクは新しいマスクで塗りつぶしレイヤーを作成します。

  ![スマートマスクをドラッグアンドドロップする](../assets/v91_drop_smart_mask.gif)
...
* <b>高度なビヘイビアーにキーボード修飾キーを使用する</b>
  ...
  * <b><b>リソース</b>へのドロップ中にCTRL</b>: 黒のマスクにレイヤースタックを含む新しいレイヤーを作成します。 マテリアルをマスクに強制的に入れるなどの場合に便利です。 または、アルファ付きのドロップダウンメニューをスキップします。
  * <b>ALT</b>を<b>サムネール</b>にドロップ中：レイヤーのレイヤースタックにドロップする場合にのみ適用されます。 ALTキーを押すと、以前のすべての効果が削除されます。 これは、さまざまなリソース、特にスマートマスクを最初に手動で取り外すことなく試す簡単な方法として使用できます。
  * <b><b>リソース</b>へのドロップ中にCTRL</b>: 黒のマスクにビューポートを含む新しいレイヤーを作成します。 リソースは、ビューポートで行った選択に基づいて設定される<b>Color ID Selection</b>効果の下に置かれます。
  * <b>ビューポート</b>にドロップ中に<b>ALT</b>：以前と同じです。リソースを強制的にデカル投影モードにします。
...
* <b>16ビット画像の可逆圧縮</b>
  ...
  この変更は主に<b>テクスチャ</b>を対象としています。これは通常、プロジェクトファイルがディスク上で非常に重くなる理由です。 平均すると、ディスク上のプロジェクトのサイズは<b>30%減から50%減</b>しました。
...
* <b>UV セット入力投影モードへの新しいUV セット</b>

  塗りつぶしレイヤー/エフェクトの新しい投影モードが<b>UVセットからUVセットへの投影</b>という名前で追加されました。 プロジェクト内のメッシュで使用可能な異なるUVに基づいてテクスチャを投影するために使用できます。 外部ツールに依存することなく、より高度なテクスチャ転送を実行できます。

  <b>UV セット0</b>は、Painterがペイントで使用するデフォルトのUVです。 追加のUV セットが使用できる場合、設定<b>ソース</b>のドロップダウン内で使用できます。

  ![UVセットプロジェクション](../assets/v91_uv_set_proj.png)
* <b>新しいプロジェクトでは、テンポラルアンチエイリアシングは既定で有効になっています</b>

  新しいプロジェクトを作成する際、ビューポートのレンダリングの質を高めるために、表示設定ウィンドウで利用可能な<b>テンポラルアンチエイリアシング</b>設定がデフォルトで有効になりました。
* <b>新しいPython APIの機能向上</b>
  ...
  * APIを介してメインビューポートのカメラを変更できるようになりました。 これには、位置や回転だけでなく、視野やアパーチャなどのその他のプロパティも含まれます。メッシュに関するカメラの配置を容易にするために、APIはシーンのバウンディングボックスも表示するようになりました。
  * 書き出しモジュールを使用して、三角形分割の有無とディスプレイスメントの有無に関わらずプロジェクトメッシュを書き出すことができるようになりました。
  * プロジェクトの書き出しテクスチャパスをAPIからも取得できるようになりました。
* <b>After Effects （ベータ版）に新しく送信</b>

  新しいsend to actionを使用して、メッシュとそのテクスチャをAfter Effectsに書き出すことができるようになりました。これにより、視覚効果を繰り返し実行するのが便利になります。 この機能を使用するには、After Effectsバージョン24.1ベータ版へのアクセスが必要です。
