...
ここでは、Windowsでグラフィックタブレットペンを使用する際の推奨事項を説明します。

## Windows Inkとは

Windows Inkは、グラフィックタブレットからスタイラスやペンなどのペンを処理するソフトウェア/サービスです。 ノート注釈やSketchpadなど、コンピューター上のペンとやり取りするための様々なアプリケーションが用意されています。
...
ペンの筆圧が正しく認識されるようにするには、グラフィックタブレットのドライバー設定でWindows Inkを有効にする必要があります。

>[!NOTE]
>
> Windows Inkは仮想マシンでサポートされていないため、グラフィックタブレットイベントはアプリケーションに転送されません。 したがって、このコンフィギュレーションではペン圧力はサポートされていません。
...
1. **デバイス**&#x200B;のウィンドウで、**ペンとWindowsインク**&#x200B;をクリックします（グラフィックタブレットが接続されている場合のみ利用可能）。\
...
| **視覚効果の表示** | 推奨： **無効**&#x200B;この設定は、ペンの様々な対話中に表示される視覚効果を制御します。 これを無効にすると、クリック時にリップル円エフェクトが非表示になります。 <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r2-column-c1_dynamic_grid_items_grid-cell_position-par_image" src="../assets/windows-pen-ripple-circle.jpg"/></div> |
| **カーソルの表示** | 推奨： **無効** |
| **一部のデスクトップアプリでペンをマウスとして使用する** | 推奨： **有効**&#x200B;この設定により、グラフィックタブレットペンは通常のマウス入力を送ることができます。 無効にすると、UIパラメーターの操作に問題が発生する可能性があります。 |
...
| **アプリがショートカットボタンの動作を上書きすることを許可する** | 推奨： **有効** |
| **ペンをストレージから取り外した後、利用可能であればインクワークスペースを表示する** | 推奨： **無効** |

## ペンとタッチの設定にアクセスする方法

コントロールパネルで、ペンとタッチの設定にアクセスできます。
...
ペイントビヘイビアーとカメラのマニピュレーションを改善するには、次の設定をおすすめします。

設定にアクセスするには、ウィンドウの&#x200B;**ペン操作**&#x200B;のいずれかをクリックし、**設定**&#x200B;をクリックします。
...
| **[ペン]ボタンを右クリックに相当する操作として使用する** | 推奨： **有効** |
| **ペンの上部を使ってインクを消します（使用可能な場合）** | 推奨： **有効** |
