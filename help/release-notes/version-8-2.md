---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/release-notes/version-8-2.html"
breadcrumb-title: ''
...
* **簡易効果のスタック**\
  以前は、特定の方法でエフェクトを組み合わせる必要がある場合（描画モードを使用する場合など）、アンカーポイントと塗りつぶしエフェクトの使用が必要でした。 フィルターに描画モードを直接適用できるようになりました。この機能により、エフェクトスタックの複雑さが大幅に軽減される可能性があります。
...
テクスチャの書き出し時に、Substanceアーカイブ(SBSAR)ファイル形式が使用できるようになりました。 SBSARは、Substanceを統合した多くのアプリケーションで開くことができるコンテナです。これにより、より高速で簡単に「プラグアンドプレイ」のカスタムテクスチャを行うことができます。

* **Substanceアーカイブ(SBSAR)をエクスポートしています**\
  **書き出しテクスチャ**&#x200B;ウィンドウで、ファイル形式の一覧からSbsar ファイル形式を指定できるようになりました。 これにより、指定したすべてのテクスチャを含む1つのSbsar ファイルが書き出されます。 出力ノードの名前とその使用方法は、選択した書き出しプリセットとそのチャンネルタイプから定義されます。

  ![](../assets/sbsar-export.png)

* **PSDおよびSbsar ファイル形式のハイブリッド書き出しプリセット**\
  書き出しプリセットで、他のすべての画像形式に加えて、PSDまたはSBSARとして出力マップを指定できるようになりました。 PSDおよびSBSARフォーマットは「コンテナ」と見なされます。つまり、複数のテクスチャがその中に保存される可能性があります。 書き出しプリセットでコンテナフォーマットとスタンドアロンイメージフォーマットの両方が指定されている場合、テンプレート内のSbsar ファイルが対象となるすべての出力がグループ化され、他の出力は個別のファイルとして書き出されます。
...
[ディスプレイの設定](../interface/display-settings/environment-settings.md)内の新しい設定により、環境マップをカメラに合わせることができ、3Dモデルの下の部分を明るくしたり、明るくしたりできます。

この新しい設定を使用するには、[表示設定](../interface/display-settings/environment-settings.md)に移動し、**環境アラインメント**&#x200B;の設定を変更します：

* **ワールド**: 環境マップはシーンに揃えられています。
* **ローカル**: 環境マップはカメラに配置されています。
...
* **ディスク上のリソースの削除と再読み込み**\
  ユーザーライブラリにあるリソースを削除、再ロード、名前変更できるようになりました（グラフやABRブラシなど、パッケージに含まれるリソースを除く）。
...
* **3Dモデルをすばやく再読み込みするための新しいアクション**\
  新しいキーボードショートカット （デフォルトでは&#x200B;**CTRL + SHIFT + R**）が追加されました。これにより、現在のプロジェクトの3Dモデルをすばやく再読み込みできます。 これにより、アセットへの反復がより簡単かつ迅速に行われます。 ソースファイルが見つからない場合、ログにエラーメッセージが表示されます。 **編集**&#x200B;メニューにもアクションが追加されました。
...
* **Substanceグラフパラメーターを既定にリセットする**\
  グラフが使用されるすべての場所（アルファ、マテリアル、フィルターなど） パラメーターをデフォルトにリセットできるようになりました。
...
* **ビューポートの個別のRGBAコンポーネントを表示する**\
  ビューポート内のチャンネルを見ると、**ディスプレイ設定/チャンネルディスプレイ**&#x200B;の下に&#x200B;**カラーチャンネル**&#x200B;という新しい設定があり、これによりRGBAコンポーネントを個別に見ることができます。 これは、テクスチャを分析したり、ユーザチャンネル内の特定のコンポーネントを分離したりするのに便利です。

  ![](../assets/channels-display-component.png)

  ![](../assets/viewport-channels.jpg){width="450px"}

* **128を超えるタイリングの塗りつぶしレイヤーと効果**\
  塗りつぶしレイヤーとエフェクトのタイリングパラメーターがソフト範囲に変更されました。 これにより、任意のタイリング値を入力できるようになりました。 スライダーのデフォルトの範囲も、ドラッグしやすくするために[-128,128]から[-32,32]に縮小されました。

  ![](../assets/fill-tiling.gif)

* **新しい16fおよび32f EXR テクスチャ書き出し設定**\
  以前は、EXRテクスチャの書き出しはインターフェイスで32fビットに強制されていましたが、実際のファイル内では16fビットデータ（半浮動小数点）が生成されていました。 現在は修正されており、16fから32fのビットを選択する可能性があります。 EXRをファイル形式として使用する古いプロジェクトと書き出しプリセットでは、古い動作を尊重するためにデフォルトで16fビットに設定されます（主に以前よりもファイルサイズの大きなファイルを作成するのを防ぐためです）。
...
* **substance\_painter.resource**\
  **substance\_painter.resource.Type**&#x200B;では、より多くの種類のリソース、特にSubstanceとPhotoshopのブラシパッケージを特定できるようになりました。\
  リソースオブジェクトは、親と子を一覧表示できるようになり、SubstanceパッケージとSubstanceグラフ間の移動などができるようになりました。
...
* **substance\_painter.event**\
  新しい&#x200B;**TextureStateEvent**&#x200B;が追加され、パラメーターのレイヤースタックの変更やその他のテクスチャセットーの変更を記録できるようになりました。 このイベントは、ペイントストロークまたはチャンネルの追加/削除によってトリガーされます。
...
* [シェーダ]ユーザチャンネルをマテリアルレイヤシェーダのRGBAとして設定できます
  ...
  マテリアルレイヤリング用シェーダー内のテクスチャセットチャンネルの設定を行う際に、チャンネルのフォーマットをデフォルト値から変更できるようになりました。 これにより、特にグレースケールのみではなくカラーユーザチャンネルを要求できます。
* [エクスポート]テクスチャをSBSARとしてエクスポートできます

  ファイル/書き出しテクスチャウィンドウを使用してテクスチャを書き出すときに、SBSAR（Substanceアーカイブ）ファイルフォーマットを選択して、一緒に再グループ化できます。 SBSARのコンテンツは、使用される出力テンプレートによって駆動されます。\
  sbsar ファイル形式は、書き出しプリセットで設定することもできます。 ハイブリッド構成（SBSAR +その他の形式）を使用する場合、SBSARを対象とするテクスチャはグループ化され、残りは同時に書き出されます。
* [書き出し] EXRファイルフォーマットの16ビットオプションを表示

  EXR テクスチャファイルの書き出し時に、書き出し設定ウィンドウで16 bit（ハーフ浮動小数）または32 bit(浮動小数)をテクスチャできるようになりました（書き出し設定と書き出しプリセットの両方）。 古いプロジェクトと古い書き出しプリセットは、古い動作を反映するためにデフォルトで16fビットに設定されます。
* [Python] テクスチャセットがいつ変更されたかを知るためのイベントを追加する

  新しい「substance\_painter.event.TextureStateEvent」により、ペイントストローク、新しいチャンネルの追加、チャンネルの削除などによってテクスチャセットが変更された日時を知ることができます。
* [Python] テクスチャセット設定でメッシュマップリソースの取得と設定を行う

  メッシュマップリソースの取得と設定を行うための新しい関数が「substance\_painter.project」モジュールに追加されました。 これらの関数は、テクスチャセット設定で参照されるメッシュマップを更新するために使用できます。
...
* [コンテンツ]新しいRobloxテンプレートを追加し、プリセットを書き出す
  ...
  新しいRobloxの「マテリアルバリアント」および「サーフェスの外観」プロジェクトテンプレートと書き出しプリセットが追加され、PBRテクスチャをRobloxに簡単に書き出せるようになりました。 テンプレートには、ファイル/新規プロジェクトウィンドウからアクセスできます。
* Substance engineを最新バージョンにアップデート(8.6.3)
* [Steam] Apple Siliconチップセット向けに最適化されたビルド(Apple M1 / M2)

**修正済み：**

* 16k exrを使用する場合のクラッシュ
* [クラッシュ] シェーダーインスタンスを削除した後にCtrl Zを押す
* [Iray]一部のシェーダでIoRが1にブロックされる
* [Win]&#x200B;[ベイク処理]一部の高いポリが読み込まれない
