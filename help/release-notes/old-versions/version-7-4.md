...
このバージョンでは、[OpenColorIO](https://opencolorio.org/) （OCIO略語）バージョン2のサポートによるカラーマネジメントが導入されています。

この新しいワークフローにより、読み込みから書き出しまで、またアプリケーション内でもカラーを管理および調整できるため、異なるビューポート間で任意のコンテンツをより簡単に一致させることができます。

* **プロジェクト設定**\
  新規プロジェクトの作成時に、カラーマネジメントを有効にできるようになりました。 既存のプロジェクトでは、プロジェクト設定を使用してカラーマネジメントを有効にすることもできます。\
  カラーマネジメントを有効にするには、**従来** （デフォルト）から&#x200B;**OpenColorIO**&#x200B;に切り替え、デフォルトの構成またはカスタムの構成のいずれかを使用します。

  ![](../../assets/cm-settings.png){width="400px"}

* **ビューポートの表示設定**\
  2Dビューと3Dビューの上部には、カラーマネジメント用の2つのコントロールがあります。\
  **色のボタン**: ビューポートの色変換を有効または無効にします。\
  **表示変形ドロップダウン**：色の変換に使用する表示変形を選択します。

  ![](../../assets/cm-viewport.jpg){width="500px"}

* **カラーピッカーの設定**\
  カラーマネジメントが有効になっている場合、カラーピッカーには新しいコントロールが表示されます。 カラーは、設定で指定された作業用カラースペースで編集されます。\
  HSV/RGBスライダの下には、作業スペースから表示カラースペースに変形した最終的なカラー値が表示されます。

  ![](../../assets/color-picker-demo.png)

  ![](../../assets/cm-picker-display-value.png)

* **カスタムカラースペースを使用したビットマップとマテリアルの読み込み**\
  マテリアルの出力の解釈など、リソースの処理方法を指定するための専用設定が利用できます。\
...
2Dおよび3Dビューをドッキング解除して、他の場所に移動できるようになりました。 例えば、2D ビューを別の画面に表示した状態で、3Dビューをメイン画面に表示できます。
...
* **カラーマネジメントと互換性があります**\
  ドッキングされていないビューには独自のカラーマネジメント表示変形があり、異なるモニターでの管理が容易になります。
...
**SpaceMouse®**&#x200B;は、3D ビューポートカメラをより直感的で使いやすい方法で操作できる3Dconnectionによるデバイスです。 このリリースでは、Painterを使用してネイティブかつ直接プラグアンドプレイできるようになりました。
...
自動ラップ解除が更新され、拡張サーフェスを使用した3Dモデルのサポートを改善する新しいオプションが追加されました。

この新しい設定&#x200B;**長すぎる可能性があるUV アイランドを分割することで、UV領域をより有効に活用する**&#x200B;という名前のUV アイランドが追加されました。
...
この新しいメソッドを使用すると、古いプラグインを新しいPython APIに簡単に移行できます。 また、Pythonでまだ表示されていない&#x200B;**ベイク処理**&#x200B;および&#x200B;**シェーダー**&#x200B;管理などの機能もロック解除されます。
