...
breadcrumb-title: ''
description: Substance 3D Painterでレイヤーのインスタンス化を使用して、複数のテクスチャセットでレイヤーを効率的に再利用する方法を説明します。
...
**レイヤーのインスタンス化**&#x200B;を使用すると、複数のレイヤーおよび[テクスチャセット](../texture-set/texture-set.md)間でレイヤーのパラメーターを同期しながら、メッシュに依存する結果を生成できます。

レイヤーインスタンスが作成されると、元のレイヤー（またはソースレイヤー）を使用して、既存のすべてのインスタンスにパラメーターが複製されます。 **ソースレイヤーのみを変更できます**。

>[!WARNING]
>
> ペイント操作（ブラシストローク、ポリゴンの塗りなど） は、ソースレイヤーが配置されているテクスチャセットでのみ機能します。 このレイヤーのインスタンスを持つ他のテクスチャセットは、ペイント操作を破棄するだけです。
...
>[!NOTE]
>
> インスタンスは、**グループ**&#x200B;を含むすべてのレイヤーから作成できます。 フォルダーのインスタンス化により、様々なテクスチャセットで複数のレイヤーを簡単に複製できます。 インスタンスフォルダー内にレイヤーを追加すると、既存のインスタンスにも複製されます。

インスタンスが作成されると、ソースレイヤーとターゲットレイヤーに新しいアイコンが表示されます。 このアイコンは、ソースレイヤーとそのインスタンス間を簡単に移動できるボタンです。テクスチャセットを手動で切り替える必要はありません（下記参照）。
...
## 複数のテクスチャセットにわたるインスタンスの作成

複数のテクスチャセットでレイヤーインスタンスを1回の操作で作成できるため、手動でコピー&amp;ペーストする必要がありません。

複数のテクスチャセットにまたがるインスタンスを作成するには、次の手順を実行します。

1. 既存のレイヤーを選択
1. レイヤーを右クリックしてコンテキストメニューを開きます
1. **[テクスチャセット間でインスタンス化]**&#x200B;を選択します
1. 新しいウィンドウで、インスタンスを受け取る必要があるテクスチャセットを確認します。
...
>[!NOTE]
>
> テクスチャセット名の横の感嘆符は、チャンネル&#x200B;**の不一致**&#x200B;を示しています。 これは、このテクスチャセットでインスタンスが作成された場合、チャンネルが見つからないため、正しくレンダリングされないことを意味します。
...
リストの&#x200B;**任意の要素**&#x200B;をクリックすると、自動的に&#x200B;**このレイヤーにジャンプ**&#x200B;します。 これにより、現在の&#x200B;**選択されているテクスチャセット**&#x200B;も自動的に&#x200B;**変更**&#x200B;され、適切なレイアウトに変更されます。

**インスタンスツリー**&#x200B;の一覧を使用すると、**依存関係**&#x200B;を同時に表示しながら、インスタンスからそのソースに&#x200B;**すばやく**&#x200B;移動できます。

## インスタンスサイクル（およびその解決方法）

サイクルは、直接または間接的にソースレイヤー自体で使用されるインスタンスです。 Substance 3D Painter エンジンでサイクル&#x200B;**を計算**&#x200B;できません。そのため、修正または削除されるまで&#x200B;**無効**&#x200B;にする必要があります。
