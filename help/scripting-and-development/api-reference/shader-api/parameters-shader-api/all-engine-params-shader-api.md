---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/scripting-and-development/api-reference/shader-api/parameters-shader-api/all-engine-params-shader-api.html"
breadcrumb-title: ''
description: Substance 3D Painterのすべてのエンジンパラメーターシェーダー API参照にアクセスし、エンジンレベルのシェーダーパラメーターを制御します。
...
テクスチャパラメーターでは、&#39;or&#39;演算子を使用してフォールバックを定義できます：
...
ここで、*テクスチャ\_タグ*&#x200B;は、以下に説明するタグの1つです。

### 文書のチャンネルタグ

シームの問題を避けるために、これらのテクスチャはすべて&#x200B;**合成**&#x200B;および&#x200B;**拡張**&#x200B;されます。

**テクスチャセットチャンネル**

*channel\_ambientocclusion* *channel\_anisotropyangle* *channel\_anisotropylevel* *channel\_basecolor* *channel\_blendingmask* *channel\_diffuse* *channel\_ディスプレイスメント* *channel\_emissive* *channel\_光沢度* *channel\_Height* *channel\_ior* *channel\_メタリック* *channel\_normal* *channel\_opacity* *channel\_reflection* *channel\_ラフネス* *channel\_scattering* *channel\_Specular* *channel\_specularlevel* *channel\_transmissive*
...
*テクスチャ\_曲率* : 曲率マップ\
*テクスチャ\_id* : ID マップ\
*テクスチャ\_normal* : 接線空間法線マップ\
*テクスチャ\_normal\_ws* : ワールド空間法線マップ\
*テクスチャ\_位置* : ワールド空間位置マップ\
*テクスチャ\_Thickness* : 厚みマップ
...
*テクスチャ\_blue\_ノイズ* ：青いノイズテクスチャ\
*テクスチャ\_environment* : 環境マップ、**mipマップ**&#x200B;で、[lib-env.glsl](../libraries-shader-api/lib-env-shader-api.md)を使用してこのライブラリを使用します
...
*カメラ\_view\_matrix* : カメラ空間からワールド空間空間への変換を表す&#x200B;*mat4*
...
*camera\_view\_matrix\_it* : *カメラ\_view\_matrix*&#x200B;の逆転置バージョン
...
*camera\_vp\_matrix\_inverse* : *投影\*&#x200B;のカメラ\_view\_matrix*行列の逆
...
*向き* :レンダリングされた面を示す&#x200B;*整数* （–1：後ろ面、0：未定義、1:フロント面）\
...
*is\_2d\_view* : 2D ビューに対してレンダリングが実行されているかどうかを示す&#x200B;*bool*
...
*シーン\_オリジナル\_半径* ：正規化前のシーンの境界球の半径を表す&#x200B;*浮動小数点*
...
*world\_direction\_direction* :ワールドカメラの向きを表す&#x200B;*vec3*
