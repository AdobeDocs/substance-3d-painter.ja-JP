...
「出力テンプレート」タブでは、新しい出力テンプレートを管理および作成できます。 出力テンプレートを使用して、書き出されたテクスチャの名前、書式および設定を変更できます。
...
一番上の行には、実行可能なすべてのテクスチャの種類が一覧表示されます。
...
>[!NOTE]
>
> 一部の型は、空の場合または同じ入力マップを共有する場合に、結合/折りたたむことができます。
> 
...
各テクスチャには、カスタムの命名規則を使用して名前を付けることができます。 いくつかのキーワードを（**$**&#x200B;ボタンを使用して）追加すると、最終的なファイルを生成したときにアプリケーションによって自動的に置き換えられます。

| キーワード | 説明 |
| --- | --- |
| **$プロジェクト** | プロジェクトファイル(.spp)の名前に置き換えられます。 |
| **$メッシュ** | メッシュファイル名（.fbxなどの入力メッシュファイル）に置き換えられます |
| **$textureset** | テクスチャの生成元のマテリアル/テクスチャセットの名前に置き換えられます。 |
| **$udim** | テクスチャの生成元のUDIM番号に置き換えられます。 |
...
入力マップの一覧に、[テクスチャセットの設定](../../interface/texture-set/texture-set-settings.md)で追加できるすべてのチャンネルが再グループ化されます。
...
| 名前 | 説明 |
| --- | --- |
| **標準 OpenGL** | ベイクされた法線とテクスチャセットの法線チャンネルのOpenGLフォーマットの合成法線マップ。 |
| **標準 DirectX** | ベイクされた法線とテクスチャセットの法線チャンネルのDirectXフォーマットの合成法線マップ。 |
| **混合AO** | ベイクしたambient occlusionとテクスチャセットのambient occlusionチャンネルのambient occlusionの組み合わせ。 |
| **Diffuse** | **Base color**&#x200B;および&#x200B;**メタリック**&#x200B;チャンネルから生成されたDiffuse テクスチャ （メタリックの領域は黒に置き換えられます）。 |
| **Specular** | **Base color**&#x200B;および&#x200B;**メタリック**&#x200B;チャンネルからSpecularテクスチャが生成されました。 |
| **光沢度** | 光沢度チャンネルの逆から生成されたラフネステクスチャ。 |
| **Unity4 Diffuse** | 非推奨。 Unity 4シェーダーに一致するように&#x200B;**Base color**&#x200B;チャンネルから生成されたDiffuseテクスチャ。 |
| **Unity4グロス** | 非推奨。 Unity 4シェーダーに一致するように&#x200B;**ラフネス**&#x200B;および&#x200B;**メタリック**&#x200B;チャンネルから生成された光沢度テクスチャ。 |
| **リフレクション** | ホワイトが誘電体マテリアルを示し、その他のカラーがメタリックマテリアルであることを示すテクスチャ。 |
| **1/ior** | 1を&#x200B;**IOR**&#x200B;値で除算したテクスチャです。 **IOR**&#x200B;は、メタリックマップから生成されます。誘電体の場合は1.4、金属（黒）の場合は100です。 |
| **光沢度<sup>2</sup>** | **光沢度**&#x200B;チャンネルの正方形バージョン(**光沢度** \* **光沢度**) |
| **f0** | フレネル0 （誘電体の場合は0.04、メタリックの場合は1.0）の反射率を含むテクスチャです。 |
