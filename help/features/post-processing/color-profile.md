...
Substance 3D Painterでは、**LUT**&#x200B;テクスチャを読み込むことによって、**カラープロファイル**&#x200B;を&#x200B;**ビューポート**&#x200B;に割り当てることができます。\
...
>  Substance 3D Painterは、可能な解像度の&#x200B;**2D テクスチャ** （フローティング）として保存された&#x200B;**3D** LUTを使用します（既定は&#x200B;**2048 x 128ピクセル** ）。 つまり、カラー操作を保存する立方体は、並べて表示されるスライスに分割されます。 技術的な詳細については、**GPU Gem**&#x200B;の記事(<http://http.developer.nvidia.com/GPUGems2/gpugems2_chapter24.html>)を参照してください。

## カラープロファイルの使用

カラープロファイルは、 Display Settingsウィンドウからロードできます。\
「**カラープロファイルを有効化**」チェックボックスをオンにして、ビューポートに影響を与え、カラープロファイルを有効にします。

![](../../assets/color-profile-ui.png)

* 「カラープロファイルを有効にする」が&#x200B;**無効**&#x200B;の場合、ビューポートのレンダリングは、マテリアルビューについては&#x200B;**sRGB**&#x200B;で実行されます（一部のチャンネルについてはリニア処理）
* 「カラープロファイルを有効にする」が&#x200B;**有効**&#x200B;の場合、ビューポートのレンダリングは、各ビュー（ソロチャンネルを含む）の&#x200B;**リニア/RAW**&#x200B;で実行されます

LUT テクスチャがリソーススロットに読み込まれる場合、 **マテリアルモード**&#x200B;にあるときにビューポートのレンダリングを操作するために使用されます。\
...
「**カラープロファイルをアクティベート**」が有効になっている場合、Substance 3D Painterはビューポートを&#x200B;**リニア**&#x200B;レンダリングに移動します。 つまり、LUTを適用する場合、リニアプロファイルから目的のターゲットにカラーを移動する必要があります。
...
### 方法2 : OpenColor IOを使用してLUT テクスチャを生成する

**OpenColor IO**&#x200B;ツールをインストールします。 次に、次の場所にあるサンプルのOCIO構成をダウンロードします： <http://opencolorio.org/downloads.html>\
...
読み込みウィンドウを開きます（またはLUTをシェルフにドラッグ&amp;ドロップします）。 Substance 3D PainterにLUTテクスチャを読み込む際は、新しいリソースソースに「**colorlut**」「**usage**」を確実に割り当ててください。 そうしないと、シェルフでリソースが正しく表示されません。
