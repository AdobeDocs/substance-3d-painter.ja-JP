---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/scripting-and-development/api-reference/shader-api/libraries-shader-api/lib-pbr-shader-api.html"
breadcrumb-title: ''
description: Substance 3D PainterのLib PBR シェーダー APIリファレンスにアクセスして、カスタムシェーダに物理的なベースのレンダリングマテリアルを作成します。
...
**パブリック関数：** *normal\_distrib* *フレネル* *G1* *visibility* *horizonFading* *pbrComputeDiffuse* *pbrComputeSpecular* *pbrComputeBRDF*
...
Specularコントリビューション計算のために作成されたサンプルの数を表すint。 画質が高くなるほど、パフォーマンスに影響します。
...
サーフェスを通して漏れ出すSpecularの反射を制御するために使用する値。
...
<https://marmosetco.tumblr.com/post/81245981087>からの地平線フェードトリック
...
見る人の目に映るマイクロファセットのSpecular反射を計算する
