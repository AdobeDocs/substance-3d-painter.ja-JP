...
breadcrumb-title: ''
description: Substance 3D PainterのUbuntuでビューポートが壊れたり応答しなくなったりしている問題を修正して、3Dを適切にレンダリングする方法を説明します。
...
バージョン11.1以降のUbuntuでSteamからPainterを実行すると、ビューポートが壊れているか、応答していないように見える場合があります。
...
1. Steam ショートカットを編集して、<b>専用のグラフィックスカードを使って実行</b>設定を無効にします。 その後、通常どおりSteamを実行します。
