---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/painting/text-resource.html"
breadcrumb-title: ''
description: Substance 3D Painterでテキストリソースを使用して、テクスチャペイントのワークフローにテキストとテキスト編集を追加する方法について説明します。
...
の<b>Text resource</b>を使用すると、特定の<b>フォントファイル</b>を使用してテクスチャにテキストを書き込むことができます。 最終的に描画されるテキストの外観を調整するために、いくつかのパラメーターを使用できます。
...
テクスチャリソースは、他のリソース（画像やSubstanceマテリアルなど）と同様に機能し、ブラシパラメーター、塗りつぶし投影、Substance画像の入力に使用できます。

リソーススロットにフォントを追加するだけでテキストリソースを作成する。 フォントをビューポートにドラッグ&amp;ドロップすることもできます。
...
| <b>パラメーター</b> | <b>説明</b> |
| --- | --- |
| <b>テキスト</b> | レンダリングするテキスト。  **注意：**&#x200B;インターフェイスのテキストフィールドでは、さまざまな種類の文字を含む一般的なフォントが使用されています。これにより、フィールドに入力された内容と、選択したフォントがテクスチャで表示できる内容が一致しない場合があります。 |
| <b>フォントサイズ</b> | フォントサイズの計算に使用するモードを指定します。 使用可能なモードは次のとおりです。<ul data-preserve-html="true"> <li data-preserve-html="true"><b>自動</b>:サイズはテキストの内容から自動的に計算され、テクスチャに合わせて調整されます。</li> <li data-preserve-html="true"><b>カスタム</b>:サイズは、専用の設定を使用して手動で制御できます。</li> </ul> |
...
| <b>パラメーター</b> | <b>説明</b> |
| --- | --- |
| <b>行の間隔</b> | フォントサイズに対するテキストの行間の距離（「行送り」）。 |
| <b>文字の間隔</b> | フォントサイズに対する、隣接する文字間の相対的な間隔です。 負の値を指定すると、間隔量が減算されます。 |
| <b>オフセット</b> | テキストの水平方向および垂直方向のオフセット。 フォントサイズに正規化されています。 |
| <b>背景の塗りつぶし</b> | テキストの背景の色です。 |
| <b>背景の不透明度</b> | 表示される背景色の量。 |
| <b>解決策</b> | テキストのレンダリングに使用するテクスチャのサイズの計算方法を指定します。 使用可能なモードは次のとおりです。<ul data-preserve-html="true"> <li data-preserve-html="true"><b>自動</b>：解像度は自動的に計算されます。</li> <li data-preserve-html="true"><b>カスタム</b>：解像度は、専用の設定を使用して手動で定義できます。</li> </ul> |
