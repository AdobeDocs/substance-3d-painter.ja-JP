---
helpx_url: "https://helpx.adobe.com/substance-3d-painter/painting.html"
breadcrumb-title: ''
description: 様々なブラシ、ツール、テクニックを使用して、Substance 3D Painterで3Dモデルにテクスチャを直接ペイントする方法を説明します。
helpx_creative_field: ""
helpx_description: Painter > Painting
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: ペインティング
user-guide-description: ''
user-guide-title: ''
source-git-commit: 9f20406f682e0e6a2e9a423e81c5ecfc7430ecfd
workflow-type: tm+mt
source-wordcount: '99'
ht-degree: 8%

---


# ペインティング

Substance 3D Painterのペイントは、手動ツールまたは自動アクションを使用して様々な方法で行うことができます。

手動ペイントツールには、ツールバー（またはその専用ショートカット）から直接アクセスできます。 ペイントツールを選択すると、そのプロパティがプロパティウィンドウに表示されます。

各ツールについて詳しくは、各ツールの専用ページを参照してください。

* [ペイントツールリスト](../painting/tool-list/tool-list.md)
* [パスツールリスト](tool-list/path.md)
* [直線](straight-line.md)
* [レイジーマウス](lazy-mouse.md)
* [対称](../painting/symmetry/symmetry.md)
* [塗りつぶしの投影](../painting/fill-projections/fill-projections.md)
* [プリセット](../painting/presets/presets.md)
* [動的ストローク](../painting/dynamic-strokes/dynamic-strokes.md)
* [チャンネルペイントの詳細設定](../painting/advanced-channel-painting/advanced-channel-painting.md)
* [ベクターグラフィック（.svgおよび.ai）](vector-graphic-svg.md)
* [テキストリソース](text-resource.md)
