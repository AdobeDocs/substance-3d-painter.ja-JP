...
Substanceブリッジプラグインを使用すると、個別のlow-polyファイルとhigh-polyファイルの書き出し、Painterへの読み込み、ベイクの設定と実行に関する長いプロセスを実行する必要がなくなります。
...
| PolyPaintを送信 | PolyPaintをテクスチャのマップに変換し、Substance内の塗りつぶしレイヤーとして適用します。このツールの上にペイントを置いて、ブレンドすることができます。 |
| スムーズ法線 | 書き出し時に正接法線をスムーズにして、ゲームエンジンがメッシュをレンダリングする方法に合わせて、ファセットされたアイテムがSubstance内でスムーズに表示されるようにします。 オフにすると、ジオメトリの実際の面が表示されます。 |
| マップの自動ベイク | モデルの到着後にSubstanceのベイク処理アルゴリズムを自動的に実行し、法線マップ、ambient occlusion、曲率、その他の詳細マップを高/低メッシュ比較から生成します。 |
| 自動ラップ解除を強制する | 受信するすべてのサブツールでSubstanceのUVラップ解除アルゴリズムをトリガーします。 モデルに既に適切なUVがある場合は、UVが上書きされるので、オフにしておきます。 |
| 再分割レベル | 送信する再分割レベルを制御します。 「現在のレベル」を選択すると、表示されているレベルのみが送信されます。 「低&amp;高」は、ベーキングの最低値と最高値の両方を送信します。ほとんどのワークフローで推奨されるオプションです。 |
| テクスチャセット | UV空間をSubstance単位で分割する方法をコントロールします。[サブツール単位]（サブツールごとに1 テクスチャセット）または[ポリゴングループ単位]（各サブツール内のPolyGroupごとに1 テクスチャセット）です。 |

Painterがモデルを受け取り、「自動ベイク」が有効になっている場合、ベイクが開始されます。 モデルの最も低いサブディビジョンはローポリメッシュとして読み込まれたメッシュで、最も高いサブディビジョンはハイポリゴンとして使用され、ディテールがベイクされます。 ZBrushはPainterよりもはるかに多くのポリゴンを処理できるため、ローポリメッシュの動作サイズが最適であることを確認してください（マシンによりますが、100万未満が最適です）。

Painterのテクスチャセットはマテリアルの割り当てを表します。 1つのテクスチャセットは1つのUVスペースに相当します。

* [サブツール単位]では、サブツールごとに1つのテクスチャセットが作成されます（すべてのサブツールパーツは同じUVスペースを共有します）。これは単純なオプションです。
* [ポリゴングループ単位]では、各サブツール内のPolyGroupごとに1つのテクスチャセットが作成され、マテリアルの割り当てをより細かく制御できます。
