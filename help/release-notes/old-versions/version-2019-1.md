---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/release-notes/old-versions/version-2019-1.html"
breadcrumb-title: ''
...
今回のリリースでは、ブラシエンジンが動的ストロークと呼ばれるものをサポートするようになりました。 この種のストロークは、新しいSubstanceバージョンがリリースされると同時にバリエーションが生まれ、新しい効果が生まれます。 アセットにペイントされた新しいブラシストロークごとに新しいマテリアルまたはアルファを使用できるようになりました。

Dynamic Stroke互換のリソースがペイントツール（ペイント、消しゴム、指先またはクローン）に読み込まれると、新しいパラメーターグループが表示されます。

![](../../assets/dynamic-stroke-ui.png)

動的ストロークでは、次のプロパティがサポートされています（Substanceグラフで表示されている場合）。
...
Substance Painterは、リアルタイムビューポートとIrayの両方で&#x200B;**ディスプレイスメント**&#x200B;と&#x200B;**メッシュテッセレーション**&#x200B;をサポートするようになりました。 両方とも、シェーダーパラメーターの下の&#x200B;**Shader Settings**&#x200B;ウィンドウで制御できます。

![](../../assets/disp-settings-1.png)

* **ソースチャンネル** :メッシュのデフォメーションの基準となるチャンネル。 デフォルトはHeightですが、ディスプレイスメントに設定することもできます。
...
「**マテリアル**」サンプルプロジェクトを（**ファイル/サンプルを読み込み**&#x200B;経由）読み込んで、この新機能をすぐに試してみてください。
...
設定をさらに簡単にするには、レイヤーを右クリックして、「**Heightの組み合わせでマスクを追加**」ショートカットを選択し、この新しいマスクをレイヤーにすばやく追加します。 このショートカットにより、Heightチャンネルの描画モードが、デフォルトの「覆い焼きリニア（追加）」ではなく「通常」に切り替わります。\
...
円形対称に対応できるように対称ツールの機能を拡張しました。 対称の設定メニューに、これを有効にする新しいモードが追加されました（コンテキストツールバーで利用可能）。

次の設定を使用できます。

* **X / Y / Z** ：円形対称で使用される対称軸の向きを制御します。
* **カウント** ：重複したポイントの数。
* **角度スパン** ：元のポイントから複製されたポイントの位置。 この設定を使用して、完全な円や4分の1などを作成できます。

また、ペイントを開始する前に設定を微調整しやすくするために、プレビューも少し追加されました。
...
塗りつぶしレイヤーと塗りつぶし効果を使用した2つの新しい投影モードが追加されました： **平面**&#x200B;と&#x200B;**球**。 また、3D投影のビヘイビアーをさらに制御するための新しいパラメーターも多数追加されました。
...
* **投影のサーフェスツール**\
  投影を簡単に操作できるようにするため、ショートカット「**Shift + W**」を使用してアクセスできる&#x200B;**Surface Tool**&#x200B;と呼ばれる3D マニピュレーターの新しいコントロールも追加されました。 コンテキストツールバーからアクセスすることもできます。 この新しいモードは、投影でのみ使用できます。

  ![](../../assets/surface-tool-toolbar.png)

  ![](../../assets/surface-tool-optim.gif)
* **投影のカリング/フェード**\
  複数の設定を使用して、投影を連続的または有限にすることができます。 カリング設定が有効になっている場合、マニピュレーターの周囲のドット付きボックスは投影のバウンディングボックスを示し、中央の線は投影が開始される場所を示します。 投影のスケールを設定することで、移動距離とフェードの開始時間を制御できます。
...
* **既存の設定を移動し、名前を変更しました**\
  これらの新しい予測により、一部の設定の動作が少し修正されました。 例えば、「**タイリング**」の名前が変更されると、「**UV ラップ**」になります。 タイリングは垂直方向または水平方向にのみ設定できるようになりました。 スケール、回転およびオフセットは、投影モード間で一貫性を保つために、「**UV変換**」という名前の新しいパラメーターグループの一部になりました。

  ![](../../assets/repeat-mode.png)

  ![](../../assets/uv-transform.png)
* **回転マニピュレーターの改善された全軸モード**&#x200B;明示的な球を描画する代わりに、下のテクスチャリングを隠さないように非表示になりました。 軸の間をクリックすると、球が選択され、すべての軸を一度に回転させることができます。\
...
* **テクスチャセットの複数選択**\
  テクスチャセット設定から複数のテクスチャセットを選択して一度に解像度を変更できるようになりました。\
  マルチセレクションモードでは、メインのエレメントという概念が残っているため、他のテクスチャセットがグレーで選択されます。 現在の選択範囲を維持したまま別のテクスチャセットに切り替える必要がある場合は、マウスの中ボタンを使用して切り替えることができます。
* **テクスチャセット一覧でクイック表示/非表示**\
...
* **書き出し時のディザリング**&#x200B;ディザリングは、ファイル形式とビット深度の横にある書き出しウィンドウの専用設定を使用して無効にできるようになりました。 ディザリングが適用される方法とタイミングの詳細については、[エクスポートドキュメント](../../export/export-window/export-window.md)を参照してください。\
  ![](../../assets/dithering.png)
* **より良いヒストグラム**\
  ヒストグラム生成ツールを修正しました。 レイヤースタックを変更すると、ヒストグラムの表示がより正確になり、ヒストグラムが正しく更新されるようになりました。\
  ![](../../assets/histogram.png)
* **レイヤーのインスタンス化の改善**\
  インスタンス化されたレイヤーの描画モードが、デフォルトの描画モードではなく「通過」に設定されるようになりました。 この描画モードを使用すると、レイヤーがテクスチャセット間でインスタンス化されている場合に、一部のエフェクトの互換性が向上します。
...
* **新しいブラシとツールプリセット**\
  このリリースでは、新しい動的ストローク機能が導入され、すぐに使用できるブラシとツールプリセットが追加されました。
...
* **13件の新しいフィルター**\
  ...
  * **MatFXピーリングペイント** ：古いペイントが剥がれ始めたような効果をシミュレートします。 このフィルターはアルファを出力するので、その下のマテリアルと簡単にブレンドできます。
  * **MatFx水滴** ：オブジェクトの表面の水滴をシミュレートします。 雨の後の車の水のように。
* **7個の新しいジェネレータ**\
  このリリースでは、いくつかの新しいジェネレータが追加されました。

  * **Ambient occlusion** :Ambient occlusionのメッシュマップを制御するマスクジェネレーターです。 マスクエディタに基づきます。
  * **ワールド空間法線** : ワールド空間法線のメッシュマップを制御するマスクジェネレーターです。 マスクエディタに基づきます。
  * **位置** ：位置のメッシュマップを制御するマスクジェネレーターです。 マスクエディタに基づきます。
  * **曲率** :曲率のメッシュマップを制御するマスクジェネレーターです。 マスクエディタに基づきます。
  * **自動ステッチャー** : UVの境界線付近、メッシュ曲率付近、またはカスタムマスク入力の周囲にステッチを作成するマスクジェネレーター。
  * **UVのテクセル密度** :メッシュの多角形のテクセル密度に基づいて、カラーのグラデーションを出力するヘルパー。
  * **UVのランダムカラー** :UV アイランドごとに（またはカスタムグラデーション入力に基づいて）ランダムカラーを生成します。
* **2件の新しい環境マップ**
...
*（2019年4月23日公開）*\
概要： **専用の新しいコンテンツ、リアルタイムとIrayのディスプレイスメントとテセレーション、比較マスク効果、円形対称、平面と球面投影法を備えた動的なストローク**
...
* [マニピュレーター] 3平面の3つの軸すべてに対する回転マニピュレーターを改善
