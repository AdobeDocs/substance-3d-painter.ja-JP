...
* <b>プロジェクトのメンテナンスを自動化するためのPython APIで利用可能</b>\
  自動アップデートワークフローもPythonに表示されました。 古いリソースを一覧表示して置き換える新しい関数が追加されました。\
...
塗りつぶしパスツールは、均一カラーで塗りつぶされた3Dモデルの表面にシェイプを作成できる新しいタイプのパスツールです。 これにより、複雑なパターンの作成が可能になります。
...
* <b>サーフェスに自動的に適応</b>\
  塗りつぶしパスツールは、平面の領域に限定されず、あらゆる種類のサーフェスに適用できます。 隙間やオブジェクトの境界を越えることができます。

  ![](../assets/filled_path_across_gaps.png)
* <b>ミラーおよびラジアル対称と互換性があります</b>\
  この新しいツールは、対称のプロパティもサポートしているため、複雑なシェイプを作成できます。
...
* <b>メッシュの多角形へのパスポイントのスナップ</b>\
  ポイントの配置を簡単にするために、新しいスナップ（マグネットアイコン）を有効にすることができます。 このオプションを使用すると、3Dモデル頂点に点を配置し、サーフェスまたはエッジに沿って移動できます。\
  スナップには3つの異なる方法があります。
...
* <b>UIの表示/非表示の表示動作を改善しました</b>\
  マニピュレーターのキーボードショートカット（<b>W</b>、<b>S</b>、または<b>D</b>）を押すと、すぐに切り替えられるようになりました。 コンテキストツールバーの専用ボタンから有効/無効にすることもできます。 この変更により、ビューポート内の他のビジュアル（パスカーブやポイントなど）を隠すことなく、すばやく表示または非表示にすることができます。

  ![](../assets/manipulator_toggle.png)
* <b>回転と拡大・縮小がパス頂点で利用できるようになりました</b>\
...
* <b>[プロパティ]ウィンドウにパス情報を表示する</b>\
  パスツールを選択すると、プロパティウィンドウに新しいセクションが表示されるようになりました。 この新しいセクションでは、パスの長さ、投影の深度、種類を切り替える操作など、パスに固有の情報と操作のグループを再度作成します。

  ![](../assets/path_properties_info.png)
* <b>ある角度から見た場合の正接版の向上</b>\
  ビュー角度によっては、カスタム正接の編集が難しい場合があります。 接線が独自のプランにコンストレインされるように、この点が変更されました。

  ![](../assets/path_tangent_angle.gif)
* <b>レイヤー間でパスの一覧を開いたままにする</b>\
  異なるペイントレイヤーやエフェクトを切り替える場合、ビューポートのパスパネルが閉じていると、他のレイヤーでも閉じたままになります。 パネルは開いたままになり、前と後ろに行きやすくなりました。

  ![](../assets/path_panel.png)
* <b>現在選択されているパス</b>にフォーカス\
  キーボードショートカット <b>F</b>を押すと、パスの編集時に3Dモデル全体ではなく、パスにフォーカスするようになりました。
* <b>バックスペース</b>のパスを削除します\
  <b>Backspace </b>のキーボードショートカットを押すと、パスをすばやく削除できます。

### 新しいSubstanceフィルターとテクスチャ生成機能

![](../assets/banner_filters.jpg)

新しいバージョンでは、いくつかの新しいフィルターとプロシージャルパターンが導入されています。
...
* <b>異方性桑原</b>\
  このフィルターは、ノイズ低減やテクスチャのスタイル設定にも使用できる[桑原フィルター](https://en.wikipedia.org/wiki/Kuwahara_filter "https://en.wikipedia.org/wiki/Kuwahara_filter")を適用します。

  ![](../assets/filter_kuwahara.jpg)
* <b>方向の距離</b>\
  これは、2D空間で特定の方向にピクセルを伸縮するための単純なフィルターです。 ブラシストロークをこする場合や、リークを発生させる場合に使用できます。
...
* <b>Triangle Grid </b>\
  ノイズの接続から構築された三角形で、ランダム度とSmoothnessをコントロールします。
* <b>タイルランダム</b>\
  建物のタイルパターンに合わせてカスタマイズされたテクスチャジェネレータ。
* <b>ボロノイとボロノイフラクタルノイズ</b>\
  これらの新しい2Dバージョンは、既に3D ノイズとして利用可能であり、2DまたはUVスペースでの作業やタイリングに使用できます。
* <b>Designer </b>のノイズを最新バージョンに更新しました\
  Painterで利用できるノイズのほとんどは、Substance 3D Designerの最新バージョンに更新されています。 ノイズパラメーターをグループ内に非表示にして編集しやすくすることはなくなりました。
...
ハイポリゴンのメッシュをローポリゴンのメッシュにベイクする場合、ケージモードを指定するときに新しい<b>自動</b>オプションを選択できるようになりました。 この新しい手法では、高ポリゴンのメッシュに最適な自動ケージメッシュを計算して、アーチファクトを防ぎます。

* <b>共通ベイクパラメーターの新しい設定</b>\
  共通のベイクパラメーターの内部では、ケージパラメーターが3つのオプションの選択肢に置き換えられました。\
  <b>距離ベース</b>：既定の正面/背面距離設定です。\
  <b>自動（試験的）</b>：新しい自動ケージです。\
...
* <b>Mac </b>で、OpenGLの代わりにMetalグラフィックAPIが使用されるようになりました\
  Painterでは、このバージョン以降、ビューポートとコンピューティングテクスチャの両方のレンダリングに、Macの<b>Metal </b>graphic APIを使用するようになりました。 このスイッチにより、アプリケーションのパフォーマンスと安定性が大幅に向上します。 また、OpenGLはMacOSで非推奨になっているため、今後は新しい機能を簡単に組み込めるようになります。
...
* <b>新しい塗りつぶしレイヤー/効果に対してのみbase colorチャンネルを有効にする</b>\
  初期設定では、新しい塗りつぶしレイヤーやエフェクトを作成すると、Base colorチャンネルのみが有効になります。 （塗りつぶしレイヤーやエフェクトを作成するリソースをドラッグ&amp;ドロップする場合は、この変更は適用されません）。\
  この変更は、コミュニティからのフィードバックに基づいており、後で無効になるチャンネルの計算をトリガーしないようにすることで、パフォーマンスを向上させました。 これは、高解像度で作業する場合やUVタイルを使用する場合の応答性に役立ちます。\
  <b>ALT </b>のキーボードショートカットを維持しながら、[Base color]ボタンをクリックすると、すべてのチャンネルをすばやく再び有効にできます。

  ![](../assets/base_color_single_channel.png)
* <b>テクスチャを書き出すためのUV タイル名の変更</b>\
  テクスチャセットリストウィンドウでは、UV タイルにカスタム名を付けることはできません。 説明とは異なり、カスタム名は専用タグ<b>$uvTileName</b>を介して書き出しプリセットで取得できます。\
  この新機能により、書き出し時にUDIM番号を具体的な名前に置き換えることができます。
...
概要： <b>メジャーリリース、新しい自動アップデート機能、塗りつぶしパスツールなどのパスの機能強化、および新しいフィルターとベイク用の実験的な自動ケージ生成</b>
...
* [Nvidia]&#x200B;[ベイク処理] Ambient occlusionベイカーの結果に斑点がある
* [クラッシュ]無効になっているテクスチャセットの表示を切り替えるために、Altをクリックするとクラッシュが発生する
* [ベイク処理]ケージは、低ポリゴンを高ポリゴンパラメータとして考慮されます
* [ベイク] ID マップベイカーのマテリアルカラーがUSDファイルフォーマットで機能しない
* [パフォーマンス] メッシュと多くの重なり合うオブジェクトとのビューポートが低く、レンダリングに時間がかかる
* [Qt]カスタムビルドのカラーピッカーにカラーマネジメント設定がない
* [ビューポート]アンチエイリアスが有効になっていると、3D マニピュレーターがちらつく
* マスクブロックのブラシ状態にある消しゴムのグレースケールスロット
* [ログ] メッシュの読み込み時に非常に長いエラーメッセージが報告されない
* [コンテンツ] Topstitches ツールプリセットー内のプリセット名のリストに誤りがあります
