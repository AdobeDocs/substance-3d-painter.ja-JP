---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/release-notes/version-10-0.html"
breadcrumb-title: ''
...
<b>Substance 3D Painter 10.0</b>では、Illustrator(.ai)ファイルのサポート、Substance 3D Assetsの統合、テキストリソースを介したフォントの読み込み、Python APIでのレイヤースタック機能の追加が行われ、QOLが向上しています。
...
この新しいバージョンでは、<b>Text resource</b>が導入されています。これは、フォントファイルを読み込んで、様々なコンテキスト（ブラシ、塗りつぶし投影、Substance画像の入力など）でテキストを書き込むためのツールです。 自分のテクスチャを飾るために
...
* <b>他のリソースと同様にフォントをドラッグ&amp;ドロップします</b>\
  フォントは、他の種類のリソースと同様にテキストリソースとして使用できます。 ドラッグ&amp;ドロップすると、塗りの投影が自動で作成されます。 また、ブラシやSubstanceフィルターへの入力としても使用できます。

  ![](../assets/v10_text_drag_drop.gif)
* <b>テキストリソースパラメーター</b>\
  テキストリソースを作成するときは、いくつかのパラメーターを調整して、テキストの外観を調整できます。縦組み文字と横組みアラインメント、自動または手動サイズ、行と文字の間隔、カラーなどです。
...
* <b>Illustrator (.Ai)ファイルのサポート</b>\
  この新しいバージョンでは、投影iファイルを読み込んでレンダリングし、ブラシ、塗りつぶし投影やSubstance画像入力でリソースとして使用できるようになりました。
...
このリリースでは、Python APIに新しいレイヤースタックモジュールが追加されました。 このAPIを使用すると、プロジェクトのレイヤースタックを制御し、高度なレイヤースタックプラグインやカスタムツールを作成できるようになります。
...
* <b>エクスポートプリセット情報のクエリ</b>\
  プリセットの書き出しがAPIに表示され、プリセット（定義済みとカスタムの両方）のリストを照会できるようになりました。 また、既存の書き出しテクスチャ APIと同様の形式でコンテンツを取得することもできます。
* <b>新たな可能性が待ち受けています。\
...
このリリースでは、法線マップペイントのワークフローが刷新されました。 ここでは、通常のブラシスタンプの重ね合わせ方法とブレンド方法を大幅に変更しました。 この変更は、ペイントフローマップに関連する問題に対処するために行われました。
...
* <b>アルファをゼロにした透明度</b>\
  アルファが0のテクスチャで作成されたブラシスタンプは、透明として描画されるようになりました。 次の例では、ブラシの形状（左）と平面の投影（右）を示しています。
...
マニピュレーターの使用を拡張するために、いくつかの機能が改善されました。

* <b>CTRLキーを押しながら精密モードを実行</b>\
  マニピュレータ上でドラッグ中にCtrlキーを押すと、新しい精度モードに入り、より細かい操作が可能になります。 この変更は、移動、回転、拡大・縮小のマニピュレーターに適用されます。\
  次に、[Ctrl]を押しながらドラッグする前と後の例を示します。

  ![](../assets/v10_ctrl_precise_mode.gif)
* <b>新しいスケールの動作</b>\
  スケールの強さは、シーンサイズではなく、現在のスケール値自体に基づいて設定されるようになりました。 これにより、特に小さい値では、相対的な変更が容易になります。 正確なモードと組み合わせることで、拡大・縮小が非常に快適になります。\
  また、0が負の値でなくなるまで縮小することも変更できます。 これにより、投影をスケールダウンして間違って反転させる問題を回避できます。

  ![](../assets/v10_translate_new.gif)
* <b>サーフェスマニピュレーターの回転を改善</b>\
  サーフェスデカールのマニピュレーターが、サーフェスの周囲をドラッグする際により安定するようになりました。 前後の変換のみを行う場合、回転は増加しません。\
  <b>新しい</b>動作と比較した<b>古い</b>動作は次のとおりです：

  ![](../assets/v10_decal_old.gif)

  ![](../assets/v10_decal_new.gif)
* <b>ドラッグアンドドロップ時に投影をカメラに揃えました</b>\
  リソースをビューポートにドラッグ&amp;ドロップすると、メッシュの表面に直接ワープ投影を作成できます。 この投影は以前は正しく回転されていませんでしたが、現在はカメラに位置合わせされています。
...
* <b>SBSAR出力のカラースペース</b>\
  従来のカラーマネジメントワークフローまたはOCIOカラーマネジメントワークフローが有効な場合、SBSAR書き出しは、各出力のプロジェクトで使用されているカラースペース名を参照するようになりました。
* <b>リソース検出の高速化</b>\
