---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-painter/release-notes/version-10-1.html"
breadcrumb-title: ''
...
* <b>新しい刺繍デカールのマテリアル</b>\
  アセットウィンドウの「マテリアル」セクションに、新しいデカールの刺繍マテリアルが追加されました。 メッシュ上のどこにでもドラッグ&amp;ドロップして、リソース（テクスチャやフォントなど）を接続すれば、新しいファブリックのディテールを簡単に作り出すことができます。
...
* <b>新しいFXAAフィルター</b>\
  この新しいフィルターを使用すると、特にレベルの後やカラー選択効果で作成されたマスクなど、ハードなエッジのエイリアスをすばやく減らすことができます。

  ![](../assets/v101_filter_fxaa.gif)
* <b>新しいハイパスフィルター</b>\
  この汎用フィルターを使用すると、グレースケールテクスチャを作成して、ディテールのソフト化、ぼかし、シャープなどの高度な効果に使用できます。
...
* <b>新しいしきい値フィルター</b>\
  2進数の黒と白のマスクをグレースケール入力から素早くシャープに生成するには、2階調化フィルターを使用します。
...
* <b>Adobe Standard Materialのテクスチャを米ドルで書き出す\
  </b>PainterからUSDファイルにテクスチャを書き出すときに、Adobe Standard Materialのプロパティを取得できるようになりました。 これにより、これらのプロパティをサポートするアプリケーションでもこれらのUSDファイルを使用できるようになります。
* <b>USDファイルからテクスチャを読み込む</b>\
  USDファイルを読み込むと、作成するプロジェクトにそのテクスチャも読み込まれるようになり、アプリケーション間の行き来が容易になりました。 USDファイルでAdobe Standard Materialが使用されている場合は、シェーダー設定も変更され、アプリケーションの結果が他のソースビューポートと一致するようになります。
* <b>Gltfの変更\
  </b>USDの更新に伴い、パリティを確保するためにGLTF形式の動作を変更する必要がありました。 gltfファイルを読み込む際に、Painterは標準マップがOpenGLフォーマットであることを前提とします。\
  一部のgltfファイルでは、代わりにDirectXフォーマットが使用される場合があります。 そのため、新しいプロジェクトウィンドウに、それを考慮した新しい設定が追加されました（標準の書式はレイヤースタックから上書きすることもできます）。
...
* <b>テッセレーションパフォーマンスの向上\
  </b>シェーダー設定でテッセレーションが有効になっている場合は、自動的に最適化されるようになりました。 画面上のピクセルより小さい三角形はテッセレーションされなくなるため、描画する三角形が少なくなり、レンダリング時間が短縮されます。\
  この変更は、視覚的な違いを生じさせることはなく、メッシュの書き出しプロセスに影響を与えることもありません。
* <b>簡易サムネイルが既定になりました</b>\
  バージョン6.2では、処理速度を上げるためにUV タイルプロジェクト用に簡素化されたサムネールが導入されましたが、通常のプロジェクトでは古い方法でレイヤーのサムネールを計算できます。 この動作は、アプリケーション設定によって制御されています。\
